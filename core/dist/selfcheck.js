...
function runResolveActorCommandChecks() {
    ...
    assert(attack.debugTrace[3]?.includes("variant=--/--") &&
        attack.debugTrace[3]?.includes("binding=--") &&
        attack.debugTrace[3]?.includes(`route=${attack.postBranchRoute}`), `attack debugTrace variant wording mismatch: ${attack.debugTrace[3]}`);
    assert(attack.debugTrace[4]?.startsWith("post-branch marker="), "attack debugTrace marker order mismatch");
    assert(attack.debugTrace[4]?.includes(`marker=${attack.postBranchTargetSource}`) &&
        attack.debugTrace[4]?.includes("pointer=shared/shared_default_target_provenance_path"), `attack debugTrace pointer meaning mismatch: ${attack.debugTrace[4]}`);
    assert(attack.debugTrace[5]?.startsWith("target terminal ") &&
        attack.debugTrace[5]?.includes(`source=${attack.targetSource}`) &&
        attack.debugTrace[5]?.endsWith(`=> ${attack.target}`), `attack debugTrace target-terminal order mismatch: ${attack.debugTrace[5]}`);
    ...
    const attackExplicit = (0, battle_1.resolveActorCommand)({
        actorIndex: 0,
        action: {
            kindId: 0x10,
            arg: 0,
    ...
    assert(attackExplicit.debugTrace[4]?.includes("marker=explicit") &&
        attackExplicit.debugTrace[4]?.includes("pointer=shared/shared_default_target_provenance_path"), `attackExplicit debugTrace marker/pointer mismatch: ${attackExplicit.debugTrace[4]}`);
    const attackSlotProbe = (0, battle_1.resolveActorCommand)({
        actorIndex: 0,
        action: {
            kindId: 0x10,
            arg: 0,
    ...
    assert(attackSlotProbe.debugTrace[4]?.includes("marker=slotIndex") &&
        attackSlotProbe.debugTrace[4]?.includes("pointer=shared/shared_default_target_provenance_path"), `attackSlotProbe debugTrace marker/pointer mismatch: ${attackSlotProbe.debugTrace[4]}`);
    ...
    assert(pointerProbe.combatDecision?.branchVariantMeaning === "candidate_family_lane_refinement_bit", `pointerProbe branchVariantMeaning mismatch: ${pointerProbe.combatDecision?.branchVariantMeaning}`);
    assert(pointerProbe.combatDecision?.branchVariantBindingStatus === "deferred_numeric_binding", `pointerProbe branchVariantBindingStatus mismatch: ${pointerProbe.combatDecision?.branchVariantBindingStatus}`);
    ...
    assert(pointerProbe.debugTrace[3]?.includes("variant=") &&
        ...
        pointerProbe.debugTrace[3]?.includes(`route=${pointerProbe.postBranchRoute}`), `pointerProbe debugTrace variant wording mismatch: ${pointerProbe.debugTrace[3]}`);
    assert(pointerProbe.debugTrace[4]?.startsWith("post-branch marker="), "pointerProbe debugTrace marker order mismatch");
    assert(pointerProbe.debugTrace[4]?.includes(`marker=${pointerProbe.postBranchTargetSource}`) &&
        pointerProbe.debugTrace[4]?.includes("pointer=candidate/candidate_entry_target_provenance_path"), `pointerProbe debugTrace pointer meaning mismatch: ${pointerProbe.debugTrace[4]}`);
    assert(pointerProbe.debugTrace[5]?.startsWith("target terminal ") &&
        pointerProbe.debugTrace[5]?.includes(`source=${pointerProbe.targetSource}`) &&
        pointerProbe.debugTrace[5]?.endsWith(`=> ${pointerProbe.target}`), `pointerProbe debugTrace target-terminal order mismatch: ${pointerProbe.debugTrace[5]}`);
    ...
    const abilityExplicit = (0, battle_1.resolveActorCommand)({
        actorIndex: 2,
        action: {
            kindId: 0x20,
            arg: 1,
    ...
    assert(abilityExplicit.debugTrace[4]?.includes("marker=explicit") &&
        abilityExplicit.debugTrace[4]?.includes("pointer=shared/shared_default_target_provenance_path"), `abilityExplicit debugTrace marker/pointer mismatch: ${abilityExplicit.debugTrace[4]}`);
    const abilitySlotProbe = (0, battle_1.resolveActorCommand)({
        actorIndex: 2,
        action: {
            kindId: 0x20,
            arg: 0,
    ...
    assert(abilitySlotProbe.debugTrace[4]?.includes("marker=slotIndex") &&
        abilitySlotProbe.debugTrace[4]?.includes("pointer=shared/shared_default_target_provenance_path"), `abilitySlotProbe debugTrace marker/pointer mismatch: ${abilitySlotProbe.debugTrace[4]}`);
    ...
    assert(matrix[4]?.debugTrace?.[4]?.includes("marker=explicit") &&
        matrix[4]?.debugTrace?.[4]?.includes("pointer=shared/shared_default_target_provenance_path"), `matrix[4] debugTrace marker/pointer mismatch: ${matrix[4]?.debugTrace?.[4]}`);
    ...
    assert(matrix[5]?.debugTrace?.[4]?.includes("marker=slotIndex") &&
        matrix[5]?.debugTrace?.[4]?.includes("pointer=shared/shared_default_target_provenance_path"), `matrix[5] debugTrace marker/pointer mismatch: ${matrix[5]?.debugTrace?.[4]}`);
}
