...
func _format_command_preview_debug(label: String, result: Dictionary) -> String:
	...
	return "%s b:%s cb:%s/%s cv:%s/%s/%s r:%s ptr:%s/%s src:%s t:%s/%s a:%s 07:%s off:%s %s/%s" % [
		label,
		branch,
		decision_branch,
		decision_branch_meaning,
...
func _format_log_preview_debug(preview: Dictionary) -> String:
	...
	return "%s k:%s a:%s s:%s b:%s cb:%s/%s cv:%s/%s/%s r:%s ptr:%s/%s src:%s t:%s/%s 07:%s off:%s src:%s/%s" % [
		label,
		kind_id,
		arg,
		slot_index,
