...
func _format_actor_resolve_debug(result: Dictionary) -> String:
	...
	var pointer_flavor_meaning := str(result.get("pointerFlavorMeaning", "--"))
	...
	return "DBG b:%s cb:%s/%s cv:%s/%s/%s r:%s\npf:%s/%s ts:%s tgt:%s k:%s a:%s s:%s c07:%s" % [
		branch,
		decision_branch,
...
func _format_command_preview_debug(label: String, result: Dictionary) -> String:
	...
	return "%s b:%s cb:%s/%s cv:%s/%s/%s r:%s ptr:%s/%s src:%s tgt:%s/%s a:%s 07:%s off:%s %s/%s" % [
		label,
		branch,
		decision_branch,
		decision_branch_meaning,
...
func _format_log_preview_debug(preview: Dictionary) -> String:
	...
	return "%s k:%s a:%s s:%s b:%s cb:%s/%s cv:%s/%s/%s r:%s ptr:%s/%s src:%s tgt:%s/%s 07:%s off:%s src:%s/%s" % [
		label,
		kind_id,
		arg,
		slot_index,
