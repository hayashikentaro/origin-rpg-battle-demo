...
func preview_current_actor_commands() -> Array:
	...
	if not abilities.is_empty():
		...
		command_inputs.append(_build_command_input("ability_slot_probe", 0, actor))
		labels.append("ABLS0:%s" % str(abilities[0].get("name", "ABILITY")))
	...
	match action_type:
		...
		"ability_slot_probe":
			...
			return {
				...
				"arg": clamped_index,
				"target": 0xFF,
				"slotIndex": min(clamped_index + 1, abilities.size()),
			}
	return {
		"kindId": 0x00,
		"arg": 0,
